...
import apps.retrieval.retrieval_workflow as retrieval_workflow
from apps.conversation.view_state import ActiveScope, ConversationViewState, FocusEntity
...
def test_build_detail_coverage_input_prefers_canonical_project_title_over_polluted_hit_title():
    merged = retrieval_workflow._build_detail_coverage_input(
        ...
        },
        {
            ...
            "facts": {"title": "?⑥씪 諛섎룄泥대Ъ吏?湲곕컲 3吏??쇰━ 寃뚯씠??媛쒕컻"},
        },
        anchor_title="1. 源遊됱?",
    )

    assert merged["title"] == "?⑥씪 諛섎룄泥대Ъ吏?湲곕컲 3吏??쇰━ 寃뚯씠??媛쒕컻"
    assert merged["title_text"] == "?⑥씪 諛섎룄泥대Ъ吏?湲곕컲 3吏??쇰━ 寃뚯씠??媛쒕컻"
...
def test_promote_unique_list_subject_anchor_supports_people_and_org_without_ids():
    people_snapshot = SimpleNamespace(
        ...
        view_id="view-people",
        items=[
            SimpleNamespace(display_rank=1, pjt_id="PJT-1", pjt_no="NO-1", rst_id=None, title_text="?꾨줈?앺듃 A", person_no=None, org_id=None, org_code=None, biz_no=None, doi=None, issn=None, researchers=["源遊됱?"], lead_org=None, participant_org=[], child_refs=[]),
            SimpleNamespace(display_rank=2, pjt_id="PJT-1", pjt_no="NO-1", rst_id=None, title_text="?꾨줈?앺듃 B", person_no=None, org_id=None, org_code=None, biz_no=None, doi=None, issn=None, researchers=["源遊됱?"], lead_org=None, participant_org=[], child_refs=[]),
        ],
    )
    org_snapshot = SimpleNamespace(
        ...
        view_id="view-org",
        items=[
            SimpleNamespace(display_rank=1, pjt_id="PJT-1", pjt_no="NO-1", rst_id=None, title_text="Project A", person_no=None, org_id=None, org_code=None, biz_no=None, doi=None, issn=None, researchers=[], lead_org="Org Alpha", participant_org=[], child_refs=[]),
            SimpleNamespace(display_rank=2, pjt_id="PJT-1", pjt_no="NO-1", rst_id=None, title_text="Project B", person_no=None, org_id=None, org_code=None, biz_no=None, doi=None, issn=None, researchers=[], lead_org="Org Alpha", participant_org=[], child_refs=[]),
        ],
    )
    ...
    assert people_anchor.title_text == "源遊됱?"
    ...
    assert org_anchor.title_text == "Org Alpha"
...
def test_node_rag_search_promotes_unique_people_child_anchor_from_list_results(monkeypatch):
    ...
    class PeopleListRetriever:
        ...
        def retrieve(self, query):
            return {
                "documents": [
                    {"title": "?⑥씪 諛섎룄泥대Ъ吏?湲곕컲 3吏??쇰━ 寃뚯씠??媛쒕컻", "pjt_id": "1711135956", "pjt_no": "2021R1F1A1057134"},
                    {"title": "愿???쇰Ц A", "pjt_id": "1711135956", "pjt_no": "2021R1F1A1057134"},
                ],
                "canonical_evidence": [
                    {
                        "context_kind": "people",
                        "ids": {"pjt_id": "1711135956", "pjt_no": "2021R1F1A1057134"},
                        "facts": {"title": "?⑥씪 諛섎룄泥대Ъ吏?湲곕컲 3吏??쇰━ 寃뚯씠??媛쒕컻", "year": 2021},
                        "roles": {"participant_researcher_name": ["源遊됱?"]},
                    },
                    {
                        "context_kind": "people",
                        "ids": {"pjt_id": "1711135956", "pjt_no": "2021R1F1A1057134"},
                        "facts": {"title": "愿???쇰Ц A", "year": 2022},
                        "roles": {"participant_researcher_name": ["源遊됱?"]},
                    },
                ],
                "render_profile": {"context_kind": "people"},
                "answer_context_text": "# 異쒖쿂 1. 源遊됱?",
                "raw_result_count": 2,
            }

    state = SimpleNamespace(
        knowledge_sufficiency=SimpleNamespace(retrieval_query="second project researchers"),
        ...
        view_state=ConversationViewState(
            active_scope=ActiveScope(
                focus=FocusEntity(kind="project", source="detail_lookup", pjt_id="1711135956", pjt_no="2021R1F1A1057134", title_text="?⑥씪 諛섎룄泥대Ъ吏?湲곕컲 3吏??쇰━ 寃뚯씠??媛쒕컻"),
                scope_kind="detail",
            ),
        ),
        request_id="rid",
        conversation_id="cid",
        question="Show the second project researchers",
        messages=[SimpleNamespace(content="Show the second project researchers")],
        request_overrides={},
    )
    ...
    result = asyncio.run(
        retrieval_workflow.node_rag_search(
            state,
            resolve_rag_queries_fn=lambda **kwargs: ("Show the second project researchers", "second project researchers", "Show the second project researchers", 1.0, False, [], False),
    ...
    assert result["view_state"].active_scope.child_anchor.title_text == "源遊됱?"
...
def test_build_detail_coverage_input_keeps_wrapper_title_for_synthetic_rows():
    merged = retrieval_workflow._build_detail_coverage_input(
        ...
        anchor_title="?⑥씪 諛섎룄泥대Ъ吏?湲곕컲 3吏??쇰━ 寃뚯씠??媛쒕컻",
    )
...
def test_build_detail_coverage_input_prefers_raw_title1_over_bilingual_title_text():
    merged = retrieval_workflow._build_detail_coverage_input(
        {
            "title": "1. 源遊됱?",
            "title_text": "?⑥씪 諛섎룄泥대Ъ吏?湲곕컲 3吏??쇰━ 寃뚯씠??媛쒕컻 Development of ternary logic gates using a single semiconducting material",
            "title1": "?⑥씪 諛섎룄泥대Ъ吏?湲곕컲 3吏??쇰━ 寃뚯씠??媛쒕컻",
        ...
        {
            "ids": {"pjt_id": "1711135956", "pjt_no": "2021R1F1A1057134"},
            "facts": {"title": "?⑥씪 諛섎룄泥대Ъ吏?湲곕컲 3吏??쇰━ 寃뚯씠??媛쒕컻 Development of ternary logic gates using a single semiconducting material"},
        },
        anchor_title="?⑥씪 諛섎룄泥대Ъ吏?湲곕컲 3吏??쇰━ 寃뚯씠??媛쒕컻 Development of ternary logic gates using a single semiconducting material",
    )
...
def test_node_rag_search_uses_anchor_locked_exact_lookup_for_detail_followup(monkeypatch):
    ...
    events = []
    view_state = ConversationViewState(
        active_scope=ActiveScope(focus=FocusEntity(kind="project", source="detail_lookup", pjt_id="1415144250", title_text="Project Alpha Detail"))
    )
...
def test_node_rag_search_detail_cache_hit_returns_evidence_context_not_direct_answer(monkeypatch):
    ...
    view_state = ConversationViewState(
        active_scope=ActiveScope(focus=FocusEntity(kind="project", source="detail_lookup", pjt_id="1415144250", title_text="Project Alpha"))
    )
    view_state.detail_cache["project:pjt_id:1415144250"] = SimpleNamespace(
        anchor=view_state.active_scope.focus,
        coverage=coverage,
...
def test_node_rag_search_does_not_force_exact_lookup_from_stale_focus_alone(monkeypatch):
    ...
    state = SimpleNamespace(
        ...
        view_state=ConversationViewState(
            active_scope=ActiveScope(focus=FocusEntity(kind="project", source="detail_lookup", pjt_id="STALE-ID", title_text="Stale Project"))
        ),
        request_id="rid",
        conversation_id="cid",
        question="? ?? ????",
...
def test_node_rag_search_prefers_rst_id_for_perf_detail_followup(monkeypatch):
    ...
    state = SimpleNamespace(
        ...
        view_state=ConversationViewState(
            active_scope=ActiveScope(focus=FocusEntity(kind="perf", source="display_snapshot", rst_id="RST-900", pjt_id="PJT-900", title_text="Perf Detail"))
        ),
        request_id="rid",
        conversation_id="cid",
        question="? ?? ??",
