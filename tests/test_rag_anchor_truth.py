...
from apps.retrieval.rag_retriever import _resolve_followup_anchor_context
...
def test_anchor_context_prefers_active_subject_before_strategy_meta():
    state = SimpleNamespace(
        intent_payload=Payload(
            normalized_intent=NormalizedIntent(
                action="detail",
        ...
        view_state=ConversationViewState(
            active_scope=ActiveScope(
                focus=FocusEntity(kind="project", source="detail_lookup", pjt_id="NEW-ID", title_text="New Project")
            )
        ),
    )
