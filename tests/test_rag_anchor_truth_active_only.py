...
from apps.retrieval.rag_retriever import get_followup_anchor_context, has_active_anchor_seed
...
def test_active_only_uses_ids_map_seed_even_without_latest_focus():
    state = SimpleNamespace(
        view_state=SimpleNamespace(active_scope=SimpleNamespace(focus=None, child_anchor=None)),
        intent_payload=SimpleNamespace(
