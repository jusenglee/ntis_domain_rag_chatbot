...
from apps.api.runtime_helpers import _state_log_summary_fields
import apps.conversation.request_facade as request_facade
from apps.conversation.request_facade import build_intent_payload
from apps.retrieval.retrieval_workflow import _normalize_display_payloads, node_rag_search
        ...
        build_intent_payload(
            ...
            view_state=ConversationViewState(
                active_scope=ActiveScope(
                    focus=FocusEntity(kind="project", source="detail_lookup", pjt_id="PJT-1", title_text="Thin Project")
                )
            ),
            request_id="rid",
...
def test_build_intent_payload_keeps_planner_locked_perf_route_without_post_merge_restore(monkeypatch):
    ...
    payload, _ = asyncio.run(
        build_intent_payload(
            ...
            view_state=ConversationViewState(
                active_scope=ActiveScope(
                    focus=FocusEntity(kind="perf", source="display_snapshot", rst_id="REP-2014-0118236988", pjt_id="1415128833", title_text="?? 7")
                )
            ),
            request_id="rid",
            cheap_precheck=lambda question: {"ids_map": {}, "years": [], "people_terms": [], "org_terms": [], "perf_tag_filters": [], "perf_types": [], "title_terms": []},
