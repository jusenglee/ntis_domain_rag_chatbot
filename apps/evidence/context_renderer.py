...
from apps.evidence.context_helpers import apply_title_preference, collect_priority_field_lines
...
def safe_map_doc(doc: Document, *, context: str) -> Optional[Dict[str, Any]]:
    """Document를 RagMapper로 매핑하되 실패 시 경고를 남기고 None을 돌려준다."""
    try:
        return RagMapper.map(doc)
    except MappingError:
        source_idx = doc.get("source_index")
        logger.exception(f"document mapping failed ({context}): source_index={source_idx}")
        return None
...
def refine_documents_rule_based(
    ...
    *,
    ...
    for doc in docs:
        ...
        mapped_doc = safe_map_doc(doc, context="refine_documents_rule_based")
        if not mapped_doc:
            continue
