...
def merge_planner_hints(
    ...
    *,
    ...
    # `org_role`은 stage2 filters에 직접 없을 수 있으므로 stage1.5의 `org_role_hint`(주관기관 역할 힌트)도 fallback으로 인식한다.
    planner_org_role = str(
        filters.get("org_role")
        or filters.get("org_role_hint")
        or getattr(qa, "org_role_hint", None)
        or getattr(qa, "org_role", None)
        or getattr(intent, "org_role", "")
        or ""
    ).strip().lower() or None
...
def apply_planner_strategy(
    ...
    *,
    ...
    def _resolve_context_owner_lock(intent_obj: Any) -> tuple[Optional[str], Optional[str]]:
    ...
    tracked_fields = (
        ...
        "ids_map", "candidate_keys", "project_key_policy", "join_resolution_policy",
        "lead_org_terms", "participant_org_terms", "org_terms", "org_role",
        "people_terms", "perf_types", "year_from", "year_to",
    )
    ...
    locked_owner_kind, context_lock_reason = _resolve_context_owner_lock(intent)
    ...
    current_project_key_policy = str(getattr(intent, "project_key_policy", "") or "").strip().lower() or None
    current_join_resolution_policy = str(getattr(intent, "join_resolution_policy", "") or "").strip().lower() or None
    ...
    patched = replace(
        ...
        project_key_policy=planner_project_key_policy or current_project_key_policy,
        join_resolution_policy=planner_join_resolution_policy or current_join_resolution_policy,
    ...
    if locked_owner_kind:
        log_event(
            ...
            context_owner_lock=locked_owner_kind,
