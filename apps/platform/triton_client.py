...
def _get_prompt_tokens(model_name: str, prompt: str) -> int:
    ...
    try:
        ...
        logger.warning(f"prompt token count failed, using fallback estimate: {e}")
...
def stream_after_assistantfinal(chunks):
    ...
    if seen and buf:
        ...
        logger.warning(
            "assistantfinal marker missing. Falling back to raw stream output."
        )
        yield buf
...
def get_triton_client() -> InferenceServerClient:
    ...
    if _triton_client is None:
        try:
            ...
            logger.info(f"client connected to {TRITON_URL}")
        except Exception as e:
