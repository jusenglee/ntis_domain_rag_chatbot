...
def build_anchor_execution_inputs(intent: NormalizedIntent) -> AnchorExecutionInputs:
    ...
    if org_role in {"lead", "lead_org", "performer", "performing"} and generic_org_terms and not lead_org_terms:
        lead_org_terms = list(generic_org_terms)
    if org_role in {"participant", "participant_org"} and generic_org_terms and not participant_org_terms:
        participant_org_terms = list(generic_org_terms)
    if org_role in {"affiliation", "affiliation_org"} and generic_org_terms and not affiliation_org_terms:
