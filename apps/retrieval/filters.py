...
def _wrap_filter_in_must_gate(filter_obj: Any) -> Optional[Any]:
    ...
    if filter_obj is None:
        return None
    should = getattr(filter_obj, "should", None) or []
    if should:
        return _build_filter(must=[filter_obj], should=None, must_not=None)
    nested_must = getattr(filter_obj, "must", None) or []
    if (
        len(nested_must) == 1
        and getattr(nested_must[0], "should", None)
        and not getattr(nested_must[0], "must", None)
        and not getattr(nested_must[0], "must_not", None)
    ):
        return filter_obj
    return _build_filter(must=[filter_obj], should=None, must_not=None)
...
def _expand_org_partial_terms(terms: List[str]) -> List[str]:
    ...
    logger.info(
        "prefix_len={} min_len={} terms_preview={} expanded_terms_preview={}",
        prefix_len,
        min_len,
        terms_norm[:4],
        expanded[:8],
