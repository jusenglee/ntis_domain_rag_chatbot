#!/usr/bin/env python3
"""NTIS RAG의 dense/sparse retrieval과 context 조립을 담당하는 모듈이다.

이 계층은 검색 결과를 모으고 prompt용 context로 가공하지만,
raw payload 전체를 그대로 LLM에 넘기지 않고 필요한 필드만 선별해 정리한다."""
...
def _filter_brief(f: Any) -> str:
    """Qdrant Filter의 must/should/must_not 구성을 로그용 짧은 문자열로 바꾼다."""
    if f is None:
        return "None"
    return (
...
def _peek(points: Any, n: int = 3) -> List[Any]:
    """상위 hit 몇 개의 id, title, score만 요약해 로그에 남긴다."""
    out: List[Any] = []
...
def _get_sparse_encoder(model_id: str = "Qdrant/bm25"):
    """fastembed sparse encoder를 lazy load하고 모델 id별로 캐시한다."""
...
def warmup_sparse_encoder(model_id: Optional[str] = None) -> bool:
    """서버 시작 시 sparse encoder를 미리 로드해 첫 요청 지연을 줄인다."""
    ...
    try:
        ...
        if enc is None:
            logger.info("sparse encoder warmup skipped (cache dir/config issue)")
            return False
...
def _encode_sparse_query(text: str, *, model_id: str) -> Optional[models.SparseVector]:
    """질의 텍스트를 Qdrant sparse vector 형태로 인코딩한다."""
    if not text:
        return None
    ...
    try:
        ...
        if enc is None:
            logger.info("sparse encoder disabled (cache dir/config issue)")
            return None
        ...
        logger.warning(f"SparseText encode failed: {e}")
        return None
...
def _qdrant_query_points_sparse(
    ...
    try:
        ...
        logger.warning(f"sparse query_points failed: {e}")
        return []
...
def _qdrant_sparse_search(
    ...
    """질의를 sparse vector로 바꾼 뒤 Qdrant sparse search로 연결한다."""
...
def _prefetch_supports_using() -> bool:
    """현재 Qdrant Prefetch 모델이 `using` 파라미터를 지원하는지 확인한다."""
...
def _supports_qdrant_query_model() -> bool:
    """현재 qdrant_client가 hybrid query에 필요한 Query 모델을 가지고 있는지 본다."""
...
def _supports_qdrant_hybrid_query() -> bool:
    """Prefetch, Fusion, Query 지원 여부를 합쳐 hybrid query_points 가능 여부를 판단한다."""
...
def _build_hybrid_query_model(
    ...
    """Qdrant 버전에 맞는 Query/QueryRequest payload를 조립한다."""
    query_model = getattr(models, "Query", None)
    if inspect.isclass(query_model):
...
def _get_fusion_rrf() -> Any:
    """지원되는 enum 형태에 맞춰 Fusion RRF 객체를 만든다."""
...
def _qdrant_hybrid_query_once(
    ...
    """dense prefetch와 sparse prefetch를 합쳐 Qdrant hybrid query_points를 한 번에 수행한다.
    
    런타임 버전이 지원하는 API 모양에 따라 using 경로와 named vector 경로를 나눠 시도한다."""
    ...
    try:
        ...
        logger.warning(f"hybrid query_points failed: {e}")
        return None

    return list(getattr(res, "points", []) or [])

def fetch_full_payloads(client: QdrantClient, collection_name: str, points):
    """부분 payload로 받은 hit를 retrieve로 다시 조회해 full payload로 보강한다."""
...
def _with_payload_selector(
    ...
    """payload mode에 따라 full payload가 아닌 필수 필드 include selector를 만든다."""
    ...
    if m in ("full",):
        try:
            ...
            logger.warning(f"_with_payload_selector failed: {e}")
            return None
    # default: min
    try:
        return models.PayloadSelectorInclude(include=merged_fields)
...
def _safe_str(x: Any, *, max_chars: Optional[int] = None) -> str:
    """None을 빈 문자열로 바꾸고 길이 제한이 있으면 로그/컨텍스트용으로 잘라낸다."""
    if x is None:
        return ""
    s = str(x)
...
def _clamp_top_k(value: Any, *, minimum: int = 0, fallback: int = 0) -> int:
    """top-k 입력을 정수 범위로 조정하고 잘못된 값은 fallback으로 대체한다."""
    try:
        val = int(value)
    except (TypeError, ValueError):
        return fallback
...
def _payload_get(pl: Dict[str, Any], key: str, default: Any = "") -> Any:
    """payload에서 dotted path를 따라 값을 꼭 집어 온다.
    
    리스트와 dict를 마구 문자열화하지 않고 scalar leaf만 이어 붙여, raw payload 구조를 그대로 prompt에 누수시키지 않는다."""
...
def normalize_query(q: str) -> str:
    """질의 앞뒤 공백과 중복 공백을 정리해 retrieval 입력으로 고정한다."""
    if q is None:
        return ""
    q = str(q).strip()
    q = _SPACE_RE.sub(" ", q)
...
def embed_query(emb: Any, text: str) -> Optional[List[float]]:
    """임베딩 객체 형태차이를 흡수하며 query embedding을 만든다."""
    if emb is None:
        return None
    try:
...
def ensure_text_index(
    ...
    """Qdrant payload 필드에 TEXT index 생성을 시도하고 실패는 디버그 로그로만 남긴다."""
    try:
...
def ensure_keyword_index(
    ...
    """Qdrant payload 필드에 KEYWORD index 생성을 시도한다."""
    try:
        client.create_payload_index(
            collection_name=collection_name,
            field_name=field_name,
...
def _attach_collection(point: Any, collection: str) -> Any:
    """hit payload에 소속 컬렉션 힌트를 심어 후단 로직이 출처를 잊지 않게 한다."""
...
def _set_payload_hint(point: Any, collection: str, vec_name: str = "") -> None:
    ...
    #return None
    """payload와 point 상위에 컬렉션/벡터 힌트를 기록한다."""
    return
...
def _make_scored_point_from_payload(
    ...
    """payload와 score를 원시 hit처럼 다룰 수 있는 ScoredPoint로 재구성한다."""
...
def _combine_filters(base: Optional[models.Filter], extra: Optional[models.Filter]) -> Optional[models.Filter]:
    """기존 Filter와 추가 Filter의 must/should/must_not를 보존하며 합친다."""
    ...
    def _get_list(f: models.Filter, name: str) -> List[Any]:
        """Filter 속성을 없으면 빈 리스트, 있으면 list로 통일한다."""
...
def dense_retrieve_hybrid_multi(
    ...
    """여러 dense vector 조회와 sparse 조회를 조합해 retrieval 결과를 만든다.
    
    hybrid_once가 가능하면 query_points 1회 경로를 우선하고, 아니면 dense/sparse를 나눠 수행한다."""
    ...
    if hybrid_once_eff:
        ...
        if hybrid_points is None:
            msg = f"failed: no hybrid response scope={contract_scope}"
            if force_hybrid_once:
                raise RuntimeError(msg)
            logger.warning(msg)
            ret = {"dense": {}, "lexical": [], "hybrid": []}
    ...
    for vec_name, emb in (emb_map or {}).items():
        ...
        try:
            try:
                ...
                logger.warning(f"query_points failed: {e}")
                continue
            ...
            t_qdrant_sum += t_q
            timings[f"dense_qdrant_{vec_name}"] = t_q
            logger.warning(f"dense query failed: col={collection_name} vec={vec_name}: {e}")
            continue
...
def _rrf_score(rank: int, rrf_k: int) -> float:
    """RRF rerank에서 rank를 score로 바꾸는 최소 공식이다."""
...
def _merge_meta(payload: Dict[str, Any]) -> Dict[str, Any]:
    """meta_basic과 meta_detail을 합쳐 context builder가 보는 통합 meta로 만든다."""
    merged: Dict[str, Any] = {}
...
def _select_meta_fields(
    ...
    """질의 단서와 output 예산에 따라 meta에서 노출할 필드만 선택한다."""
    ...
    def _normalize_year_value(value: Any) -> Any:
        """YYYY-MM-DD 형태의 날짜를 연도만 보이도록 가볍게 정규화한다."""
        if value is None:
            return value
        s = str(value).strip()
        if not s:
...
def _format_meta_lines(meta: Dict[str, Any], keys: List[str]) -> str:
    """선택된 meta 필드를 `- key: value` 라인 묶음으로 만든다."""
    lines: List[str] = []
    for k in keys:
        v = meta.get(k)
        if v is None or v == "":
...
def build_context_docstyle(
    ...
    """retrieval hit를 LLM에 넘기는 docstyle context로 바꾼다.
    
    output_type와 fieldset을 보고 meta/content 노출량을 조절하며, token budget를 넘으면 본문을 다시 줄여서 맞춘다."""
    out_chunks: List[str] = []
...
def build_context(points: Sequence[Any], max_items: int = _CTX_MAX_ITEMS) -> Tuple[str, List[Dict[str, Any]]]:
    """기존 호출처를 위한 backward-compatible context builder 엔트리포인트다."""
...
def build_context_mixed(
    ...
    """현재 표준 context builder로 build_context_docstyle에 인자를 그대로 위임한다."""
