...
def _load_run_rag_ab_compare() -> Any:
    ...
    from apps.retrieval.rag_pipeline import run_rag_ab_compare as runtime_run_rag_ab_compare
...
def get_followup_anchor_context(state: Any, *, active_only: bool = False) -> Dict[str, Any]:
    ...
    active_subject = get_active_subject_entity(view_state)
    ...
    def first_value(key: str) -> Optional[str]:
        ...
        if not active_only:
            candidates.append(getattr(active_subject, key, None))
    ...
    title_candidates = []
    if not active_only:
        title_candidates.append(getattr(active_subject, "title_text", None))
    ...
    if not active_only:
        entity_kind_candidates.append(getattr(active_subject, "kind", None))
    ...
    payload: Dict[str, Any] = {
        "present": bool(entity_key or title_text),
            "anchor_source": (
                ...
                or (None if active_only else getattr(active_subject, "source", None))
...
class CustomRAGRetriever(BaseModel):
    ...
    def retrieve(self, query: str) -> Dict[str, Any]:
        ...
        source_refs = list(getattr(res_m, "source_refs", None) or [])
        citation_registry = getattr(res_m, "citation_registry", None)
