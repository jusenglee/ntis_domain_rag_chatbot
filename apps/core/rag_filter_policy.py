...
def resolve_collection_server_filter(
    *,
    ...
    with_org_must_gate_fn: Callable[[Any], Any],
    log_lookup_ids_empty_fn: Callable[..., None],
    log_project_key_filter_fn: Callable[..., None],
) -> Any:
    ...
    def _relation_lookup_filter_for_col() -> Any:
        ...
        if not context.relation_lookup_enforce or not context.relation:
            return None
        route = get_relation_route_fn(context.relation)
        if route is None:
            return None
        if col == route.hop1_col:
            ...
            return None
        base_filter = build_tag_only_filter_fn(tag_filters_local) if tag_filters_local else None
    ...
    def _build_soft_filter_for_col() -> Any:
        ...
        if col == context.col_project:
            ...
            if apply_name_filters and (context.people_filter or context.participant_org_filter or context.org_filter):
                ...
                base_filter = and_filter_fn(base_filter, tag_filter_local)
            ...
            if context.project_tag_filter:
                base_filter = and_filter_fn(base_filter, context.project_tag_filter)
    ...
    def _apply_extra_filters(base_filter: Any) -> Any:
        ...
        combined = and_filter_fn(relation_filter, base_filter) if relation_filter else base_filter
        if col in (context.col_project, context.col_perf) and context.year_range_filter:
            combined = and_filter_fn(combined, context.year_range_filter)
        if col == context.col_perf and context.perf_type_filter:
            combined = and_filter_fn(combined, context.perf_type_filter)
        if col == context.col_perf and context.perf_followup_filter:
            combined = and_filter_fn(combined, context.perf_followup_filter) if combined else context.perf_followup_filter
        return combined
    ...
    pjt_filter = build_project_id_filter_fn(context.pjt_ids, context.pjt_nos)
    if pjt_filter is not None:
        log_project_key_filter_fn(
            project_key_filter_type=project_key_filter_type,
            pjt_id_count=len(context.pjt_ids),
            pjt_no_count=len(context.pjt_nos),
        )
        combined = and_filter_fn(pjt_filter, base_filter_lookup) if base_filter_lookup else pjt_filter
        return _apply_extra_filters(with_org_must_gate_fn(combined))

    if col == context.col_perf and context.perf_tag_filter:
        combined = and_filter_fn(context.perf_tag_filter, base_filter_lookup) if base_filter_lookup else context.perf_tag_filter
        return _apply_extra_filters(with_org_must_gate_fn(combined))

    if col == context.col_project and context.relation == ("people", "project") and context.people_filter and lookup_has_ids:
        tag_filter_local = build_tag_only_filter_fn(["IRD_NAI_PJT_INFO"])
        combined = and_filter_fn(and_filter_fn(tag_filter_local, context.people_filter), base_filter_lookup) if base_filter_lookup else and_filter_fn(tag_filter_local, context.people_filter)
        return _apply_extra_filters(with_org_must_gate_fn(combined))
    ...
    if col == context.col_project and context.org_terms and context.base_route not in ("project", "org", "people") and lookup_has_ids:
        if context.org_role == "participant":
            combined = and_filter_fn(context.participant_org_filter or context.org_filter, base_filter_lookup) if base_filter_lookup else (context.participant_org_filter or context.org_filter)
            return _apply_extra_filters(with_org_must_gate_fn(combined))
        if context.org_filter:
            combined = and_filter_fn(context.org_filter, base_filter_lookup) if base_filter_lookup else context.org_filter
            return _apply_extra_filters(with_org_must_gate_fn(combined))
    ...
    if col == context.col_project:
        ...
        if context.base_route == "org":
            tag_filter_local = build_tag_only_filter_fn(["IRD_NAI_PJT_INFO"])
            base_filter = and_filter_fn(tag_filter_local, context.participant_org_filter or context.org_filter) if (context.participant_org_filter or context.org_filter) else tag_filter_local
            return _apply_extra_filters(with_org_must_gate_fn(base_filter))
    ...
    if col == context.col_perf and context.base_route == "perf":
        ...
        if combined_filter is not None:
            return _apply_extra_filters(with_org_must_gate_fn(combined_filter))
    return _apply_extra_filters(with_org_must_gate_fn(base_filter_lookup))
