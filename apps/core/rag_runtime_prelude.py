...
def build_runtime_prelude(*, request: RuntimePreludeRequest, runtime: RuntimePreludeRuntime, timings: Dict[str, Any]) -> RuntimePreludeResult:
    ...
    contract_violations = validate_planner_contract(mode=plan.mode, head=ctx.base_route, relation=ctx.relation, target_cols=list(ctx.target_collections or plan.target_collections or []), ids_map=getattr(ctx, "ids_map", None), relation_target_cols=relation_target_cols_for_contract, join_key_mode=resolved_join_key_mode, candidate_keys=getattr(ctx, "candidate_keys", None), project_key_policy=getattr(ctx, "project_key_policy", None))
