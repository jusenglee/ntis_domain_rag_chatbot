...
from apps.api.services.followup_anchor import (
...
from apps.api.services.view_state import ActiveScope, ChildEntityRef, ConversationViewState, FocusEntity
...
def _latest_snapshot(view_state: Optional[ConversationViewState]) -> Optional[Any]:
    ...
    return active_scope.result_set or (getattr(view_state, "latest_display_snapshot", None) if view_state is not None else None)


def _scope_focus_entity(view_state: Optional[ConversationViewState]) -> Optional[FocusEntity]:
    active_scope = _get_active_scope(view_state)
    return active_scope.focus or (getattr(view_state, "latest_focus_entity", None) if view_state is not None else None)


def _active_anchor_entity(view_state: Optional[ConversationViewState]) -> Optional[FocusEntity]:
    active_scope = _get_active_scope(view_state)
    return active_scope.child_anchor or (getattr(view_state, "latest_focus_entity", None) if view_state is not None else None)
    ...
    if child_reference["ambiguous"]:
        ...
        return ScopeDecision(
            ...
            clarification_payload=_build_clarification_payload(
                ...
                message=f"현재 상세 안에서 어떤 {subject}를 가리키는지 확인해 주세요.",
