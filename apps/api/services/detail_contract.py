...
_ENTITY_LABELS = {"project": "Project", "perf": "Performance", "people": "Researcher", "org": "Organization"}
...
def make_entity_cache_key(anchor: FocusEntity) -> str:
    for kind, key, value in (
        (anchor.kind, "pjt_id", anchor.pjt_id),
        (anchor.kind, "pjt_no", anchor.pjt_no),
        (anchor.kind, "rst_id", anchor.rst_id),
        (anchor.kind, "person_no", anchor.person_no),
        (anchor.kind, "org_id", anchor.org_id),
        (anchor.kind, "org_code", anchor.org_code),
        (anchor.kind, "biz_no", anchor.biz_no),
        (anchor.kind, "doi", anchor.doi),
        (anchor.kind, "issn", anchor.issn),
        (anchor.kind, "doc_id", anchor.doc_id),
    ):
        if value:
            return f"{kind}:{key}:{value}"
    return f"{anchor.kind}:title:{anchor.title_text or 'unknown'}"
...
def compute_detail_coverage(document: Dict[str, Any], *, anchor: Optional[FocusEntity] = None) -> DetailCoverage:
    ...
    core_profile = {
        ...
        "title": _text(doc.get("title") or doc.get("title_text") or facts.get("title") or meta_basic.get("kor_pjt_nm") or meta_basic.get("title")),
        ...
        "rst_id": _text(doc.get("rst_id") or ids.get("rst_id") or meta_detail.get("rst_id") or meta_basic.get("rst_id")),
        "person_no": _text(doc.get("person_no") or doc.get("hm_id") or ids.get("person_no")),
        "org_id": _text(doc.get("org_id") or meta_detail.get("org_id") or meta_basic.get("org_id")),
        "org_code": _text(doc.get("org_code") or doc.get("org_cd") or meta_detail.get("org_code") or meta_basic.get("org_code")),
        "biz_no": _text(doc.get("biz_no") or doc.get("org_no") or meta_detail.get("biz_no") or meta_basic.get("biz_no")),
        "doi": _text(doc.get("doi") or ids.get("doi") or meta_detail.get("doi") or meta_basic.get("doi")),
        "issn": _text(doc.get("issn") or ids.get("issn") or meta_detail.get("issn") or meta_basic.get("issn")),
        "year": _text(facts.get("year") or doc.get("stan_yr") or meta_basic.get("stan_yr") or meta_detail.get("stan_yr")),
...
def render_detail_answer(coverage: DetailCoverage, *, requested_fields: Optional[Iterable[str]] = None) -> str:
    ...
    label = _ENTITY_LABELS.get(entity_kind, "Item")
    lines = []
    ...
    for field in ("pjt_id", "pjt_no", "rst_id", "person_no", "org_id", "org_code", "biz_no", "doi", "issn"):
        if _present(core.get(field)):
            lines.append(f"{field.upper()}: {core.get(field)}")
    if _present(core.get("year")):
        lines.append(f"YEAR: {core.get('year')}")
    if _present(core.get("lead_org")):
        lines.append(f"LEAD_ORG: {core.get('lead_org')}")
    if _present(core.get("participant_org")):
        lines.append(f"PARTICIPANT_ORG: {', '.join(core.get('participant_org') or [])}")
    if _present(core.get("researchers")):
        lines.append(f"RESEARCHERS: {', '.join(core.get('researchers') or [])}")
    if coverage.detail_level == "profile_only":
        lines.append(_decode_text('\ud604\uc7ac \uc790\ub8cc\ub85c\ub294 \ucd94\uac00 \uc0c1\uc138 \ud56d\ubaa9\uc744 \ucda9\ubd84\ud788 \ud655\uc778\ud558\uae30 \uc5b4\ub835\uc2b5\ub2c8\ub2e4.'))
        return '\n'.join(lines)
    rich = coverage.rich_detail
    for field in ("summary", "goal", "period", "budget", "perf_type", "affiliation"):
        if _present(rich.get(field)):
            lines.append(f"{field.upper()}: {rich.get(field)}")
    if _present(rich.get("outputs")):
        outputs = rich.get("outputs") or []
        if isinstance(outputs, list):
            lines.append(f"OUTPUTS: {', '.join(str(v) for v in outputs if str(v).strip())}")
        else:
