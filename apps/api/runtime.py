...
async def initialize_app_runtime(app: FastAPI, *, config: AppRuntimeConfig) -> None:
    ...
    adequacy_gate: Any = AdequacyGate(llm=dialogue_llm) if adequacy_enabled else None
    logger.info(
        f"Agentic pipeline — PlannerAgent + ToolExecutor with "
    ...
    # 6) LangGraph 컴파일 — agentic 그래프 (dialogue_agent → entity_resolver → planner_loop 흐름)
