...
|---|---|---|
| request payload / follow-up | `apps/conversation/request_facade.py` | 질문 해석과 anchor lock이 시작되는 지점 |
| planner runtime | `apps/planner/planner_runtime.py` | stagewise planner 진입점 |
| planner contract | `apps/planner/planner_contract.py` | `SEARCH/LOOKUP/JOIN` 계약 |
| retrieval workflow | `apps/retrieval/retrieval_workflow.py` | 실제 검색, display snapshot, detail cache |
| retriever adapter | `apps/retrieval/rag_retriever.py` | runtime query와 RAG pipeline 연결부 |
| canonical evidence | `apps/evidence/canonical_evidence.py` | raw 검색 결과를 정리된 근거로 바꾸는 지점 |
| view/session state | `apps/conversation/view_state.py` | display snapshot / latest focus / detail cache |
