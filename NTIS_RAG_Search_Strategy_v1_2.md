...
*(SEARCH / LOOKUP / JOIN + LLM Planner )*
...
A) **ID 기반 질문**  
- 예:
  - “1711015550 과제 상세” → `ids_map.pjt_id`
  - “PJT-2020-XXXX 과제” → `ids_map.pjt_no`
  - “REP-2025-0001 보고서” → `ids_map.rst_id`
  - “10.XXXX/XXXX DOI” → `ids_map.doi`
  - “ISSN 1234-5678” → `ids_map.issn|eissn|pissn`
  - “특허 10-XXXXXXX” → `ids_map.patent_reg_no` (또는 출원번호면 `patent_app_no`)
- 요구: **해당 컬렉션에 맞는 식별자 키가 ids_map에 반드시 포함**되어야 함
...
플래너 출력은 **실행 레이어가 그대로 수행하는 “단일·불변 Strategy”** 이며,
아래 **필수 키를 항상 포함**해야 한다. (빈 값이면 `null`/`{}`/`[]`로 채움)
...
- `strategy_version`는 **항상 `"v2"`** 로 고정한다.
- `mode=JOIN`이면:
  - `relation`은 `project_perf|perf_project` 중 하나여야 함
  - `join_key_mode`는 `instance|group` 중 하나여야 함(누락 시 실행 레이어는 기본값 `instance`로 보정 가능하나, 플래너는 명시 권장)
  - `ids_map`에는 **프로젝트 조인키가 반드시 있어야 함**
    - `join_key_mode=instance` → `ids_map.pjt_id` 필요 (`pjt_no` only는 금지)
    - `join_key_mode=group` → `ids_map.pjt_no` 필요 (`pjt_id` only는 금지)
- `ids_map`에서 `pjt_id`와 `pjt_no` **동시 제공(혼합)은 금지**(정확도/재현성 저하 + 계약 위반).
- `target_cols`는 **실제 Qdrant 컬렉션명**으로 명시한다.
  - 기본 허용: `ntis_project_v1`, `ntis_perf_v1` (운영 allowlist 기준)
  - 실행 레이어의 allowlist는 **검증 목적**이며, `target_cols`를 **재결정/재작성하지 않는다**(단일·불변 계약 유지).

### 8.3 ids_map 키 allowlist(권장: 이 외 키 사용 금지)

- project 식별자:
  - `pjt_id`, `pjt_no`
- perf 식별자(성과 단건/정확조회):
  - `doi`, `issn`, `eissn`, `pissn`, `patent_reg_no`, `patent_app_no`, `paper_id`, `perf_id`, `rst_id`
- people/org 식별자(정확 게이트):
  - `person_no`, `org_id`, `org_code`, `biz_no`

> 원칙: **사람/기관 기반 질의는 대부분 LOOKUP**으로 해결하며(문서 내부 객체 필터),
> people/org를 먼저 확정하는 2-hop JOIN(Hop1)은 기본 비활성(오염/불확실성 증가).

### 8.4 버전 관리(운영/디버깅 핵심)

- `strategy_version`(플래너 계약 버전): **입력/출력 스키마 계약**의 버전  
  - 키 추가/삭제, 값 enum 변경 등 “플래너 JSON 스키마”가 바뀌면 증가
- `policy_version`(내부 검색 정책 버전): **preset/topK/rerank/필터 컴파일 정책**의 버전  
  - 가중치/상한/리랭킹 정책 변경 등 “검색 정책”이 바뀌면 증가  
  - 로그에는 `policy_version`으로 기록(재현성/회귀분석용)

### 8.5 권장(선택)

- `output_type`(detail|summary|list|stats): action과 함께 출력 형식을 명시하면 운영/프롬프트 관리가 쉬워짐
- `filters`는 설계 문서의 filters 키(기관/인물/연도/태그 등)만 포함하며, 실행 레이어는 이를 Qdrant Filter로 **컴파일**한다.
...
- [ ] 플래너 output이 v2 계약(필수 키/enum/`strategy_version="v2"`)을 만족하는가  
- [ ] `target_cols`가 allowlist 내 실제 컬렉션명(`ntis_project_v1`/`ntis_perf_v1`)으로 내려오는가  
...
- [ ] (운영 기본) promotion은 비활성(`RAG_PROMOTION_MODE=disable`)인가  
- [ ] (계약 기본) 검색 실패 시 chat fallback은 비활성(`RAG_FORCE_FALLBACK_CHAT=0`)인가  
