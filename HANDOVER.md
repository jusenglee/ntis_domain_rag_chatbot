...
### 5.4 브랜치 구조 (v1 운영 / v2 개발)
- **`고도화`(현재 브랜치) = v1, 현재 운영 중인 소스.** v2 반영 전까지 **운영·유지보수 대상** (`apps/api` 계열, 이 문서가 설명하는 코드).
- **`판단-검색-에이전트` = v2, 개발 중.** v1을 갈아엎고 새 기능 추가, **향후 v1 대체 예정** (`apps/pipeline` agentic 구조 — 별도 문서 세트 보유).
- **`misp` = 사내 저장소.** 팀 서버(`miso` 리모트)엔 운영 v1(`고도화`)이 올라가 있다.
- 즉 정본은 "미합의"가 아니라 **운영=고도화(v1) → 차기=판단-검색-에이전트(v2)** 로 정해져 있다. v2가 완성·검증되면 운영으로 승격·배포하는 것이 로드맵. (`main`은 2026-04 이후 방치 상태이므로 배포 기준으로 쓰지 말 것.)
