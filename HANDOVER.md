...
### 4.4 브랜치 구조 (v1 운영 / v2 개발)
- **`고도화` = v1, 현재 운영 중인 소스.** v2 반영 전까지 **운영·유지보수 대상** (`apps/api` 계열).
- **`판단-검색-에이전트`(현재 브랜치) = v2, 개발 중.** v1을 갈아엎고 새 기능 추가, **향후 v1 대체 예정** (`apps/pipeline` agentic).
- **`misp` = 사내 저장소.** 팀 서버(`miso` 리모트)엔 운영 v1(`고도화`)이 올라가 있고, v2는 `origin`에만 있다.
