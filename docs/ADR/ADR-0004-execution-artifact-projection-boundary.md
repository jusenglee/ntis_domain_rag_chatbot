...
- `apps/api/services/retrieval_workflow.py`의 `node_rag_search()`는 다음 책임을 한 함수 안에서 함께 처리한다.
...
- `apps/api/services/result_set.py`의 `RetrievalBundle`은 아래 성격이 다른 내용을 동시에 들고 있다.
...
- update: `apps/api/services/retrieval_workflow.py`
- update: `apps/api/services/result_set.py`
- update: `apps/api/services/answer_generation.py`
...
`apps/core/rag_pipeline.py`와 `apps/api/services/rag_result_assembly.py`는 현재도 크지만, 이 ADR의 첫 단계에서는 직접 분해 대상으로 잡지 않는다. 먼저 route/retrieval/post-retrieval boundary를 고정한 뒤 다음 단계 debt로 다루는 편이 안전하다.
...
- `apps/core/rag_pipeline.py`와 `apps/api/services/rag_result_assembly.py`의 대형 시그니처는 별도 debt로 남는다.
