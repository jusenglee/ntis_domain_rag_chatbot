...
이 문서는 사용자 질의가 들어온 뒤 NTIS Domain RAG 시스템이 어떤 단계를 거쳐 동작하는지 설명하는 기준 문서입니다.
목적은 다음 두 가지입니다.

- 운영자와 개발자가 요청 1건의 처리 흐름을 단계별로 추적할 수 있게 한다.
- 문서만 보고 시스템 다이어그램을 일관된 vocabulary로 그릴 수 있게 한다.

## 1. 전체 요청 라이프사이클

사용자 질의는 아래 순서로 처리됩니다.
...
- planner artifact입니다.
- 질의 해석 과정의 planner 출력이며 execution truth가 아닙니다.
- debug나 planner drift 진단에는 유용하지만, 실행 레이어는 이 값을 직접 truth처럼 사용하면 안 됩니다.

### `intent_payload.normalized_intent`

- retrieval intent의 canonical input입니다.
- query understanding 단계가 만든 정규화 결과이며 retrieval workflow의 기본 입력입니다.
- `question_analysis`보다 아래 레이어에서 우선합니다.

### `strategy`

- execution strategy truth입니다.
- planner merge, contract validation, runtime prelude를 통과한 최종 실행 전략입니다.
- runtime, orchestration, response observability는 이 값을 우선 봐야 합니다.

### `canonical_evidence`

- retrieval result normalization truth입니다.
- raw retrieval payload를 prompt에 직접 넣지 않고, canonical schema로 정규화한 결과입니다.
- `pjt_id`, `pjt_no`, 기관 역할 의미를 보존해야 합니다.

### `render_profile`

- presentation contract입니다.
- `output_type`과 route/mode에 맞는 context shape를 결정합니다.
- answer wording을 임의로 정하는 값이 아니라 evidence presentation shape를 정하는 값입니다.
...
- broad query, exact lookup, relation query의 retrieval intent 후보를 정리한다.
...
동작:
- retrieval result를 사람이 읽을 수 있는 응답으로 변환한다.
- stream event, answer merge, follow-up UX를 제공한다.
- conversation history와 canonical context snapshot을 함께 저장/로드할 수 있다.
- 이전 문맥의 retrieval anchor와 planner seed는 canonical evidence / render profile를 우선 기준으로 사용한다.
...
- 운영 요약 로그는 `REQ.SUMMARY`에 execution strategy를 우선 기록한다.
...
- 과도한 hard filter를 두지 않는다.
- ranking과 retrieval breadth가 중요하다.

### LOOKUP

- identifier 또는 강한 제약 기반 조회에 사용한다.
- 정확 filter와 field constraint가 중요하다.
- 사람/기관 이름 기반 질의도 기본적으로 LOOKUP 계열에서 시작한다.

### JOIN

- project-perf 관계형 질의에 사용한다.
- relation과 join seed가 모두 있어야 한다.
- hop1 / hop2의 execution policy와 runtime key materialization이 중요하다.
