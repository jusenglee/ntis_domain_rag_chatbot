...
- `EmptyStreamContentError`(빈 스트림)는 삼키지 않고 상위 레이어로 전파한다(fail-fast).
- deadline 초과/무컨텐츠 시 non-stream fallback(`ainvoke`)으로 대체하지 않으며, 빈 문자열을 반환할 수 있다.
- truncation 안내문은 “실제 content 일부가 스트리밍된 경우”에만 부착한다(폴백 여부와 무관).
